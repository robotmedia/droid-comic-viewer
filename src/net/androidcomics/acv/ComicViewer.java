...
import net.robotmedia.activity.ExtendedActivity;
...
public class ComicViewer extends ExtendedActivity implements  OnGestureListener, GestureDetector.OnDoubleTapListener, ComicViewListener {
	...
	private class LoadComicTask extends AsyncTask<String, Object, Comic> {
		...
		protected Comic doInBackground(String... params) {
			...
			if (result != null) {
				HistoryManager.getInstance(ComicViewer.this).remember(new File(path));
			}
		...
		protected void onPreExecute () {
			progressDialog = new ACVDialogFactory(ComicViewer.this).createLoadProgressDialog();
			progressDialog.show();
	...
	public void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if (requestCode == Constants.SCREEN_BROWSER_CODE && resultCode == RESULT_OK) {
			int index = data.getIntExtra(Browse.POSITION_EXTRA, mScreen.getIndex());
	...
	private void shareScreen() {
		...
		task.setRelativeTempPath(ACVConstants.TEMP_PATH);
	...
	private void showMenu() {
		ComicViewer.this.openOptionsMenu();
	}
	
	private void startBrowseActivity() {
		if (isComicLoaded()) {
			final Intent intent = new Intent(this, Browse.class);
			intent.putExtra(Browse.POSITION_EXTRA, mScreen.getIndex());
			final String comicID = comic.getID();
			intent.putExtra(Browse.EXTRA_COMIC_ID, comicID);
			startActivityForResult(intent, Constants.SCREEN_BROWSER_CODE);
		}
	}

	private void startSDBrowserActivity() {
		Intent myIntent = new Intent(this, SDBrowser.class);
	...
	protected void removePreviousComic(boolean emptyTemp) {
		...
		if (emptyTemp) {
			File tempDirectory = new File(Environment.getExternalStorageDirectory(), ACVConstants.TEMP_PATH);
	...
	public void onAnimationEnd(AbstractComicView comicView) {
		this.adjustCornersVisibility(true);
	}

	public void onAnimationStart(AbstractComicView comicView) {
